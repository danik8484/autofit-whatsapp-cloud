...
def find_best_food(query: str, coach_id: str = "", _depth: int = 0):
    ...
    # "ריבה" + תיאור שאינו וריאציה אמיתית במאגר ("ריבה טבעית"/"ריבה ביתית") → ברירת המחדל
    # של דני ("ריבה" → מעדן ארבעה פירות סאנט דלפור). הווריאציות האמיתיות ("ריבה דלה",
    # "ריבה פירות יער לייט") מזוהות בחיפוש רגיל ונשמרות; רק צירוף בלי התאמה נופל לברירת המחדל.
    # בודקים מול הטקסט המדויק שדני כתב (לא אחרי נרמול — הנרמול חותך "לייט"→"ייט" ושובר שכן).
    _rq = query.strip()
    _rw = _rq.split()
    if len(_rw) > 1 and _rw[0] == "ריבה":
        _rc = _canon_cook(_rq.lower())
        _rfoods = search_food(_rq, coach_id)
        if not any(_canon_cook(f.get("food_name", "").lower()).startswith(_rc) for f in _rfoods):
            _rb, _ra = find_best_food("ריבה", coach_id, _depth + 1)
            if _rb:
                return _rb, _ra
...
if __name__ == "__main__":
    ...
    meals_json_query = _pop_arg("--meals-json")   # 🍽️🤝 07-08: תפריט-מתאמן כ-JSON (הדלת של TheDani)
    ...
    # 🍽️🤝 07-08 (הדלת של TheDani): תפריט המתאמן כ-JSON — status: ok / no_data / no_conn.
    #    קריאה בלבד; שום שינוי. TheDani משתמש בזה ל"בדקתי, X לא בתפריט שלך".
    if meals_json_query:
        try:
            uid_mj, full_name_mj, _ = find_user(meals_json_query)
            if not uid_mj or uid_mj == "MULTIPLE":
                print(json.dumps({"status": "no_data",
                                  "why": "multiple" if uid_mj == "MULTIPLE" else "user-not-found"},
                                 ensure_ascii=False))
                sys.exit(0)
            _meals_mj = []
            for _m_mj in (get_user_meals(uid_mj) or []):
                _foods_mj = [{"food_name": (f.get("food_name") or "").strip(), "quantity": f.get("quantity")}
                             for f in (_m_mj.get("mealFoods") or _m_mj.get("new_meal_food") or [])
                             if (f.get("food_name") or "").strip()]
                _meals_mj.append({"meal_name": _m_mj.get("meal_name") or "", "foods": _foods_mj})
            print(json.dumps({"status": "ok", "name": full_name_mj, "meals": _meals_mj}, ensure_ascii=False))
        except Exception as _e_mj:
            print(json.dumps({"status": "no_conn", "why": str(_e_mj)[:120]}, ensure_ascii=False))
        sys.exit(0)
