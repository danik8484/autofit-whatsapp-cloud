...
// 🌉 07-06: הקו העסקי (7107645253) עבר מ-GreenAPI לגשר-Baileys (yellowCard חסם את GreenAPI על המספר).
//    הגשר מחקה את ה-API של GreenAPI (/waInstance{id}/{method}/{token}) → מספיק להחליף HOST+TOKEN.
//    אם WA_HOST מוגדר — כל קריאות ה-BIZ (lastOutgoingMessages/getStateInstance/getContact...) עוברות
//    דרך הגשר עם BRIDGE_TOKEN. אם לא — GreenAPI כרגיל (נפילה-לאחור, תאימות-אחורה).
const WA_HOST   = process.env.WA_HOST || '';
const BIZ_TOKEN = WA_HOST ? (process.env.BRIDGE_TOKEN || '') : process.env.GREEN_API_TOKEN_BIZ;
const BIZ_BASE  = BIZ_ID
  ? (WA_HOST ? `${WA_HOST}/waInstance${BIZ_ID}` : `https://api.green-api.com/waInstance${BIZ_ID}`)
  : null;
  ...
  while (true) {
    ...
    try {
      ...
      if (silentMin >= SYNC_ALERT_MIN && !syncAlertSent && BIZ_GROUP) {
        ...
        let state = null;
        try {
          const resp = await fetch(`${BIZ_BASE}/getStateInstance/${BIZ_TOKEN}`);
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (state && state !== 'authorized') {
          ...
          await fetch(`${BIZ_BASE}/reboot/${BIZ_TOKEN}`);
        }
      } else {
...
async function cacheVoiceNow(chatId, idMessage, name, directUrl) {
  ...
  // 🌉 גשר-Baileys: ה-downloadUrl כבר מגיע ברשומה (directUrl) — משתמשים בו ישירות (אין downloadFile בגשר).
  const url = directUrl || await downloadFileRetry(chatId, idMessage, { label: '(נכנס)' });
  if (!url) return;
...
async function processBizBody(body) {
  ...
  if (body.typeWebhook === 'incomingMessageReceived' && msg?.typeMessage === 'audioMessage') {
    ...
    if (msgId && BIZ_BASE && BIZ_TOKEN) {
      ...
      cacheVoiceNow(vChatId, msgId, vName, body.downloadUrl);  // downloadUrl מהגשר (אם קיים) → תמלול בלי downloadFile
    }
    return;
  }
...
// ─── BIZ Incoming Polling loop — lastIncomingMessages (הקלטות + תמונות-קבוצה מלקוחות) ──
// 🌉 07-07: נפרד לגמרי מ-bizPollLoop (הפקודות) — כשל כאן לא נוגע בקליטת הפקודות.
//   מחזיר לזרם את ההודעות-הנכנסות שהגיעו דרך /webhook-biz לפני המעבר לגשר:
//   audioMessage → תמלול (cacheVoiceNow), imageMessage בקבוצת-המלצות → Drive (tryHandleRecsImage).
//   רץ רק כשהקו העסקי על הגשר (WA_HOST) — בגרין-אפי incoming הגיע ב-webhook.
const BIZ_IN_WATERMARK_FILE = require('path').join(DATA_DIR, 'biz_in_watermark.json');
async function bizIncomingPollLoop() {
  if (!WA_HOST || !BIZ_BASE || !BIZ_TOKEN) return;
  const nowTs = Math.floor(Date.now() / 1000);
  let lastTs = nowTs;
  const seenIds = new Map();
  try {
    const saved = JSON.parse(require('fs').readFileSync(BIZ_IN_WATERMARK_FILE, 'utf8'));
    if (saved && saved.ts && (nowTs - saved.ts) < 600) {
      lastTs = saved.ts;
      if (saved.ids && typeof saved.ids === 'object') for (const [id, ts] of Object.entries(saved.ids)) seenIds.set(id, ts);
    }
  } catch (e) { /* אין קובץ — התחלה רגילה */ }
  console.log('[biz-in-poll] starting, watermark=', new Date(lastTs * 1000).toISOString());
  while (true) {
    await new Promise(r => setTimeout(r, 10000)); // כל 10 שניות
    try {
      const resp = await fetch(`${BIZ_BASE}/lastIncomingMessages/${BIZ_TOKEN}`);
      if (!resp.ok) continue;
      const msgs = await resp.json();
      if (!Array.isArray(msgs)) continue;
      const cycleNow = Math.floor(Date.now() / 1000);
      ...
      for (const msg of ordered) {
        ...
        maxTs = Math.max(maxTs, msg.timestamp);
        if (msg.idMessage) seenIds.set(msg.idMessage, msg.timestamp);
        const tm = msg.typeMessage || '';
        // רק סוגים שאוטופיט מטפל בהם נכנסים — הקלטה (תמלול) / תמונה (קבוצת-המלצות). השאר מיותר.
        if (tm !== 'audioMessage' && tm !== 'imageMessage') continue;
        // קבוצה: _rjid שומר את ה-@g.us המקורי (הגשר ממיר chatId ל-@c.us). tryHandleRecsImage דורש @g.us.
        const rjid = msg._rjid || '';
        const chatId = rjid.endsWith('@g.us') ? rjid : (msg.chatId || '');
        const messageData = (tm === 'imageMessage')
          ? { typeMessage: 'imageMessage', fileMessageData: { downloadUrl: msg.downloadUrl || '', caption: msg.textMessage || '' } }
          : { typeMessage: 'audioMessage' };
        await processBizBody({
          typeWebhook: 'incomingMessageReceived',
          idMessage: msg.idMessage,
          senderData: { chatId, chatName: '', sender: msg.chatId || '' },
          messageData,
          timestamp: msg.timestamp,
      ...
      const pruneBefore = cycleNow - 600;
      for (const [id, ts] of seenIds) { if (ts < pruneBefore) seenIds.delete(id); }
      try { require('fs').writeFileSync(BIZ_IN_WATERMARK_FILE, JSON.stringify({ ts: lastTs, ids: Object.fromEntries(seenIds) })); } catch (e) {}
    } catch (e) {
      console.error('[biz-in-poll error]', e.message);
    }
  }
}
...
// ─── 🍽️🤝 07-08 (הכרעת-דני): הדלת של TheDani ─────────────────────────────────
// TheDani בודק תפריט-מתאמן ומוסיף מאכל — רק אחרי שדני אישר את הכרטיס בפאנל שלו.
// אבטחה: טוקן ייעודי (THEDANI_TOKEN); בלי טוקן מוגדר הדלת סגורה. כתיבה: רק "תוסיף ל...".
app.post('/api/thedani', (req, res) => {
  const expected = process.env.THEDANI_TOKEN || '';
  const got = req.headers['x-thedani-token'] || (req.body && req.body.token) || '';
  if (!expected || got !== expected) return res.status(403).json({ ok: false, error: 'forbidden' });
  const b = req.body || {};
  let args = null;
  if (b.meals) {
    args = ['--meals-json', String(b.meals).slice(0, 60)];
  } else if (b.request) {
    const r = String(b.request).slice(0, 200);
    if (!/^תוסיף ל/.test(r)) return res.status(400).json({ ok: false, error: 'only-add-allowed' });
    args = [r];
  } else {
    return res.status(400).json({ ok: false, error: 'meals-or-request-required' });
  }
  const script = path.join(__dirname, 'autofit_api.py');
  const proc = spawn('python3', [script, ...args]);
  let out = '', err = '';
  ...
  const killer = setTimeout(() => { try { proc.kill(); } catch (_) {} }, 75000);
  proc.on('close', (code) => {
    clearTimeout(killer);
    console.log(`[thedani-door] ${args[0] === '--meals-json' ? 'תפריט' : 'הוספה'}: ${args.join(' ').slice(0, 80)} → code=${code}`);
    res.json({ ok: code === 0, output: out.trim().slice(0, 4000), error: err.trim().slice(0, 300) });
  });
});
